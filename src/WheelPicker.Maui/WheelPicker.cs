...
public partial class WheelPicker : ContentView
{
    ...
    private const double AxisLockThreshold = 8;
    ...
    // axis lock (prevents horizontal pan from moving wheel)
    private bool _axisDecided;
    private bool _axisIsVertical;
    ...
    private void OnPanUpdated(object? sender, PanUpdatedEventArgs e)
    {
        ...
        switch (e.StatusType)
        {
            ...
            case GestureStatus.Running:
                ...
                if (!_axisDecided)
                {
                    if (Math.Abs(dy) < AxisLockThreshold && Math.Abs(dx) < AxisLockThreshold)
                        return;

                    _axisIsVertical = Math.Abs(dy) >= Math.Abs(dx);
                    _axisDecided = true;
                ...
                double deltaTotalY = dy - _lastPanTotalY;
                ...
                double totalDeltaItems = -(dy / ItemHeight);
